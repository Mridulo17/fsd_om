...
@section('title') @lang('sub_department.index_title') @endsection
...
@section('thead')
    <tr>
        ...
        <th>@lang('sub_department.label_name')</th>
        <th>@lang('sub_department.label_bn_name')</th>
        <th width="5%">@lang('sub_department.label_status')</th>
        <th width="12%">@lang('sub_department.label_action')</th>
    </tr>
@endsection

@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('sub_department.index_title')@endslot
        @slot('create_button')
            <a href="{{route('sub_department.create')}}" class="btn btn-primary btn-sm waves-effect waves-light modal_lg_button">
                <i class="fa fa-plus-circle"></i> @lang('sub_department.create_button_title')
            </a>
        @endslot
    @endcomponent
...
@section('script')
    ...
    <script type="text/javascript">
        let datatable_columns = [
            ...
            {data: 'department.bn_name',name: 'department.bn_name',},
            {data: 'name',name: 'name',},
        ...
        $('#datatable').DataTable({
            ...
            ajax: {
                url: '{{ route('sub_department.index') }}',
                type: 'get',
                dataType: 'JSON',
                cache: true,
            },
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            $('#deleted_list_datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('sub_department.deleted_list') }}',
                    type: 'get',
                    dataType: 'JSON',
                    cache: true,
                },
        ...
        function statusChange(id){
            statusUpdate(id, '{{route('sub_department.status')}}')
        }
    </script>
