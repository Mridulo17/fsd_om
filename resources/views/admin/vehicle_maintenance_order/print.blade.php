...
@section('content')
    ...
    <table style="border: 2px solid #000; height: 900px;">
        <tr>
            ...
            <td>@lang('vehicle_maintenance_order.serial_number')</td>
            <td>@lang('vehicle_maintenance_order.subject')</td>
            <td>@lang('vehicle_maintenance_order.order_giving_employee_id')</td>
            <td>@lang('vehicle_maintenance_order.memorandum_number')</td>
            <td>@lang('vehicle_maintenance_order.memorandum_date')</td>
        </tr>
        @foreach($vehicle_maintenance_order->vehicleMaintenanceDetails as $key => $vehicle_maintenance_order)
            <tr>
                <td class="text-center" style="width: 15px;">{{ \App\Helpers\ENTOBN::convert_to_bangla($loop->iteration) }}.</td>
                <td class="">{{ $vehicle_maintenance_order->serial_number }}</td>
                <td class="">{{ $vehicle_maintenance_order->subject }}</td>
                <td>{{ $vehicle_maintenance_order->employee->bn_name }} [{{ $vehicle_maintenance_order->employee->old_pin }}]  [{{ $vehicle_maintenance_order->employee->new_pin }}]</td>
                <td class="">{{ $vehicle_maintenance_order->memorandum_number }}</td>
                <td class="">{{ $vehicle_maintenance_order->memorandum_date }}</td>
            </tr>
        @endforeach
    </table>
@endsection
