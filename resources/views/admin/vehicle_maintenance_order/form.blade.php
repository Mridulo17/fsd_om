            ...
            <div  class="border border-secondary mb-6 p-3 my-2">
                ...
                <div class="row" >
                    ...
                    <div class="col-sm-12 col-md-4">
                        ...
                        <label for="order_giving_employee_id" class="form-label">@lang('vehicle_maintenance_order.order_giving_employee_id',['model' => trans('vehicle_maintenance_order.vehicle_maintenance_order')])</label>
                        <div class="form-group">
                            <select v-model="row.order_giving_employee_id" :name="'vehicle_maintenance_orders['+index+'][order_giving_employee_id]'" class="{{$error_class}} form-control">
                                <option value="">@lang('common.select_one')</option>
{{--                                @foreach($employees as $key => $employee)--}}
{{--                                    <option value="{{$employee->id}}">{{$employee->bn_name}}</option>--}}
{{--                                @endforeach--}}
                                <option v-for="(employee,index) in visitor_employees" :value="index"    :selected="index == row.order_giving_employee_id">@{{employee}}
                                </option>
                            </select>
                            @if ($errors->has('vehicle_maintenance_orders'))
                                <p class="text-danger">{{$errors->first('vehicle_maintenance_orders')}}</p>
                            @endif
                        </div>
                    </div>
...
{{--                        <div class="form-group">--}}
{{--                            <input pattern="/^(0[1-9]|1\d|2\d|3[01])\-(0[1-9]|1[0-2])\-(1|2)\d{3}$/"--}}
{{--                                   placeholder="dd-mm-yyyy" v-model="row.memorandum_date" class="{{$error_class}} form-control"--}}
