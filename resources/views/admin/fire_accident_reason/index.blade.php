...
@section('title') @lang('fire_accident_reason.index_title') @endsection
...
@section('thead')
    <tr>
        <th width="5%">#</th>
        <th>@lang('fire_accident_reason.label_name')</th>
        <th>@lang('fire_accident_reason.label_bn_name')</th>
        <th width="5%">@lang('fire_accident_reason.label_status')</th>
        <th width="12%">@lang('fire_accident_reason.label_action')</th>
    </tr>
@endsection

@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('fire_accident_reason.index_title')@endslot
        @slot('create_button')
            <a href="{{route('fire_accident_reason.create')}}" class="btn btn-primary btn-sm waves-effect waves-light modal_lg_button">
                <i class="fa fa-plus-circle"></i> @lang('fire_accident_reason.create_button_title')
            </a>
        @endslot
    @endcomponent
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('#datatable').DataTable({
            ...
            ajax: {
                url: '{{ route('fire_accident_reason.index') }}',
                type: 'get',
                dataType: 'JSON',
                cache: true,
            },
        ...
        $('#deleted_list_datatable').DataTable({
            ...
            ajax: {
                url: '{{ route('fire_accident_reason.deleted_list') }}',
                type: 'get',
                dataType: 'JSON',
                cache: true,
            },
        ...
        function statusChange(id){
            statusUpdate(id, '{{route('fire_accident_reason.status')}}')
        }
    </script>
