...
@section('title') @lang('common.index',['model' => trans('master_roll_report.master_roll_report')]) @endsection
...
@section('thead')
    <tr>
        <th width="5%">#</th>
        <th>@lang('master_roll_report.memorandum_no')</th>
        ...
        <th>@lang('master_roll_report.thana_id')</th>
...
@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('common.index',['model' => trans('master_roll_report.master_roll_report')])@endslot
        @slot('create_button')
            <a href="{{route('master_roll_report.create')}}" class="btn btn-primary btn-sm waves-effect waves-light">
                <i class="fa fa-plus-circle"></i> @lang('common.create',['model' => trans('master_roll_report.master_roll_report')])
            </a>
        @endslot
    @endcomponent
    ...
    <ul class="nav nav-tabs" id="Tab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active text-primary" id="all_btn_tab" data-bs-toggle="tab" data-bs-target="#all" type="button" role="tab" aria-controls="all" aria-selected="true">@lang('common.index_list',['model' => trans('master_roll_report.master_roll_report')])</button>
        </li>
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            <li class="nav-item" role="presentation">
                <button class="nav-link text-primary" id="deleted_btn_tab" data-bs-toggle="tab" data-bs-target="#deleted_list" type="button" role="tab" aria-controls="deleted_list" aria-selected="false">@lang('common.deleted_list',['model' => trans('master_roll_report.master_roll_report')])</button>
            </li>
        @endif
    </ul>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('#datatable').DataTable({
            ...
            ajax: {
                url: '{{ route('master_roll_report.index') }}',
                type: 'get',
                dataType: 'JSON',
                cache: true,
            },
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            $('#deleted_list_datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('master_roll_report.deleted_list') }}',
                    type: 'get',
                    dataType: 'JSON',
                    cache: false,
                },
